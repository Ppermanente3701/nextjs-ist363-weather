const openWeatherApiKey = process.env.NEXT_PUBLIC_OPENWEAHTERMAP_API_KEY;
...
export async function getWeatherDataByLatLon(position) {
  ...
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${lattitude}&lon=${longitude}&appid=${openWeatherApiKey}&units=imperial`
  );
  const data = await response.json();
  return data;
}
